...
import { GraduationCap, BookOpen, Target, Trophy, Loader2, ArrowRight } from "lucide-react";
          ...
          <div className="mt-4 space-y-2">
            ...
            {d.available_exams.map((e) => (
              <div key={e.id} className="flex items-center justify-between border border-border p-3 rounded-sm" data-testid={`avail-exam-${e.id}`}>
                ...
                <Link to="/app/exams"><Button size="sm" variant="outline" className="rounded-sm">{e.attempted ? "View" : "Start"} <ArrowRight className="w-3 h-3 ml-1" /></Button></Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid-card p-5">
          ...
          <div className="mt-4 space-y-2">
            ...
            {d.recent_attempts.map((a) => (
              <Link to={`/app/result/${a.id}`} key={a.id} className="block border border-border p-3 rounded-sm hover:bg-muted/30">
